...
export const LessonRecord = ({ lesson, onClose, onMarkAsCompleted }) => {
  return (
    <Dialog open={!!lesson} onClose={onClose}>
      ...
      <DialogActions>
        ...
        <Button onClick={() => onMarkAsCompleted(lesson.id)} color="secondary" variant="contained">
          Урок проведён
        </Button>
      </DialogActions>
    </Dialog>
...
const ScheduleTeacher = () => {
  ...
  const markLessonAsCompleted = (lessonId) => {
    console.log(`Урок с ID ${lessonId} отмечен как проведённый`);
    // Здесь можно добавить логику для отправки данных на сервер
    // Например, через axios.post('/api/mark-as-completed', { lessonId })
    closeLessonRecord();
  };
